...
//! BackgroundSubtractorSuBSENSE::m_nDescDistThresholdOffset
#define BGSSUBSENSE_DEFAULT_DESC_DIST_THRESHOLD_OFFSET (3)
//! defines the default value for
//! BackgroundSubtractorSuBSENSE::m_nMinColorDistThreshold
...
//! defines the default value for
//! BackgroundSubtractorSuBSENSE::m_nRequiredBGSamples
#define BGSSUBSENSE_DEFAULT_REQUIRED_NB_BG_SAMPLES (2)
//! defines the default value for
//! BackgroundSubtractorSuBSENSE::m_nSamplesForMovingAvgs
#define BGSSUBSENSE_DEFAULT_N_SAMPLES_FOR_MV_AVGS (100)

/*!
        Self-Balanced Sensitivity segmenTER (SuBSENSE) change detection
   algorithm.

        Note: both grayscale and RGB/BGR images may be used with this extractor
   (parameters are adjusted automatically). For optimal grayscale results, use
   CV_8UC1 frames instead of CV_8UC3.

        For more details on the different parameters or on the algorithm itself,
   see P.-L. St-Charles et al., "Flexible Background Subtraction With
   Self-Balanced Local Sensitivity", in CVPRW 2014.
...
class BackgroundSubtractorSuBSENSE : public BackgroundSubtractorLBSP {
 public:
  ...
  BackgroundSubtractorSuBSENSE(
      float fRelLBSPThreshold =
          BGSSUBSENSE_DEFAULT_LBSP_REL_SIMILARITY_THRESHOLD,
  ...
  virtual void refreshModel(float fSamplesRefreshFrac,
                            bool bForceFGUpdate = false);
  //! primary model update function; the learning param is used to override the
  //! internal learning thresholds (ignored when <= 0)
  virtual void operator()(cv::InputArray image, cv::OutputArray fgmask,
                          double learningRateOverride = 0);
 ...
 protected:
  //! absolute minimal color distance threshold ('R' or 'radius' in the original
  //! ViBe paper, used as the default/initial 'R(x)' value here)
  ...
  //! number of different samples per pixel/block to be taken from input frames
  //! to build the background model (same as 'N' in ViBe/PBAS)
  const size_t m_nBGSamples;
  //! number of similar samples needed to consider the current pixel/block as
  //! 'background' (same as '#_min' in ViBe/PBAS)
  ...
  //! background model pixel color intensity samples (equivalent to 'B(x)' in
  //! PBAS)
  std::vector<cv::Mat> m_voBGColorSamples;
  //! background model descriptors samples
  std::vector<cv::Mat> m_voBGDescSamples;

  //! per-pixel update rates ('T(x)' in PBAS, which contains pixel-level
  //! 'sigmas', as referred to in ViBe)
  cv::Mat m_oUpdateRateFrame;
  //! per-pixel distance thresholds (equivalent to 'R(x)' in PBAS, but used as a
  //! relative value to determine both intensity and descriptor variation
  //! thresholds)
  cv::Mat m_oDistThresholdFrame;
  //! per-pixel distance variation modulators ('v(x)', relative value used to
  //! modulate 'R(x)' and 'T(x)' variations)
  cv::Mat m_oVariationModulatorFrame;
  //! per-pixel mean distances between consecutive frames ('D_last(x)', used to
  //! detect ghosts and high variation regions in the sequence)
  cv::Mat m_oMeanLastDistFrame;
  //! per-pixel mean minimal distances from the model ('D_min(x)' in PBAS, used
  //! to control variation magnitude and direction of 'T(x)' and 'R(x)')
  cv::Mat m_oMeanMinDistFrame_LT, m_oMeanMinDistFrame_ST;
  //! per-pixel mean downsampled distances between consecutive frames (used to
  //! analyze camera movement and control max learning rates globally)
  cv::Mat m_oMeanDownSampledLastDistFrame_LT,
      m_oMeanDownSampledLastDistFrame_ST;
  //! per-pixel mean raw segmentation results (used to detect unstable
  ...
  //! a lookup map used to keep track of unstable regions (based on segm. noise
  //! & local dist. thresholds)
  ...
  //! used for blinking px detection)
