...
BackgroundSubtractorLOBSTER::BackgroundSubtractorLOBSTER(
    float fRelLBSPThreshold, size_t nLBSPThresholdOffset,
    size_t nDescDistThreshold, size_t nColorDistThreshold, size_t nBGSamples,
...
void BackgroundSubtractorLOBSTER::initialize(const cv::Mat& oInitImg,
                                             const cv::Mat& oROI) {
  ...
  if (oInitImg.type() == CV_8UC3) {
    ...
    if (!cv::countNonZero((voInitImgChannels[0] != voInitImgChannels[1]) |
      ...
      std::cout
          << std::endl
          << "\tBackgroundSubtractorLOBSTER : Warning, grayscale images should "
             "always be passed in CV_8UC1 format for optimal performance."
          << std::endl;
  }
  cv::Mat oNewBGROI;
  ...
  if (m_nImgChannels == 1) {
    ...
    for (size_t t = 0; t <= UCHAR_MAX; ++t)
      m_anLBSPThreshold_8bitLUT[t] = cv::saturate_cast<uchar>(
          (t * m_fRelLBSPThreshold + m_nLBSPThresholdOffset) / 2);
    ...
    for (size_t t = 0; t <= UCHAR_MAX; ++t)
      m_anLBSPThreshold_8bitLUT[t] = cv::saturate_cast<uchar>(
          t * m_fRelLBSPThreshold + m_nLBSPThresholdOffset);
    for (size_t nPxIter = 0, nModelIter = 0; nPxIter < m_nTotPxCount;
         ++nPxIter) {
      if (m_oROI.data[nPxIter]) {
        m_aPxIdxLUT[nModelIter] = nPxIter;
...
void BackgroundSubtractorLOBSTER::refreshModel(float fSamplesRefreshFrac,
                                               bool bForceFGUpdate) {
  // == refresh
...
void BackgroundSubtractorLOBSTER::operator()(cv::InputArray _image,
  ...
  if (m_nImgChannels == 1) {
    for (size_t nModelIter = 0; nModelIter < m_nTotRelevantPxCount;
      ...
      while (nGoodSamplesCount < m_nRequiredBGSamples &&
             nModelIdx < m_nBGSamples) {
        const uchar nBGColor = m_voBGColorSamples[nModelIdx].data[nPxIter];
        {
          const size_t nColorDist = L1dist(nCurrColor, nBGColor);
          if (nColorDist > m_nColorDistThreshold / 2) goto failedcheck1ch;
          LBSP::computeGrayscaleDescriptor(
              oInputImg, nBGColor, nCurrImgCoord_X, nCurrImgCoord_Y,
              m_anLBSPThreshold_8bitLUT[nBGColor], nCurrInputDesc);
          const size_t nDescDist = hdist(
              nCurrInputDesc,
              *((ushort*)(m_voBGDescSamples[nModelIdx].data + nDescIter)));
          if (nDescDist > m_nDescDistThreshold) goto failedcheck1ch;
          nGoodSamplesCount++;
        }
      failedcheck1ch:
        nModelIdx++;
      ...
      if (nGoodSamplesCount < m_nRequiredBGSamples)
        ...
        if ((rand() % nLearningRate) == 0) {
          ...
          ushort& nRandInputDesc =
              *((ushort*)(m_voBGDescSamples[nSampleModelIdx].data + nDescIter));
          LBSP::computeGrayscaleDescriptor(
              oInputImg, nCurrColor, nCurrImgCoord_X, nCurrImgCoord_Y,
              m_anLBSPThreshold_8bitLUT[nCurrColor], nRandInputDesc);
        ...
        if ((rand() % nLearningRate) == 0) {
          ...
          const size_t nSampleModelIdx = rand() % m_nBGSamples;
          ushort& nRandInputDesc =
              m_voBGDescSamples[nSampleModelIdx].at<ushort>(nSampleImgCoord_Y,
                                                            nSampleImgCoord_X);
          LBSP::computeGrayscaleDescriptor(
              oInputImg, nCurrColor, nCurrImgCoord_X, nCurrImgCoord_Y,
              m_anLBSPThreshold_8bitLUT[nCurrColor], nRandInputDesc);
          m_voBGColorSamples[nSampleModelIdx].at<uchar>(
              nSampleImgCoord_Y, nSampleImgCoord_X) = nCurrColor;
        }
      }
    }
  } else {  // m_nImgChannels==3
    ...
    for (size_t nModelIter = 0; nModelIter < m_nTotRelevantPxCount;
      ...
      while (nGoodSamplesCount < m_nRequiredBGSamples &&
             nModelIdx < m_nBGSamples) {
        const ushort* const anBGDesc =
            (ushort*)(m_voBGDescSamples[nModelIdx].data + nDescIterRGB);
        const uchar* const anBGColor =
            m_voBGColorSamples[nModelIdx].data + nPxIterRGB;
        ...
        for (size_t c = 0; c < 3; ++c) {
          ...
          LBSP::computeSingleRGBDescriptor(
              ...
              m_anLBSPThreshold_8bitLUT[anBGColor[c]], anCurrInputDesc[c]);
          const size_t nDescDist = hdist(anCurrInputDesc[c], anBGDesc[c]);
          if (nDescDist > nCurrSCDescDistThreshold) goto failedcheck3ch;
          nTotColorDist += nColorDist;
          nTotDescDist += nDescDist;
        }
        if (nTotDescDist <= nCurrDescDistThreshold &&
            nTotColorDist <= nCurrColorDistThreshold)
          nGoodSamplesCount++;
      failedcheck3ch:
        nModelIdx++;
      }
      if (nGoodSamplesCount < m_nRequiredBGSamples)
        ...
        if ((rand() % nLearningRate) == 0) {
          const size_t nSampleModelIdx = rand() % m_nBGSamples;
          ushort* anRandInputDesc = ((
              ushort*)(m_voBGDescSamples[nSampleModelIdx].data + nDescIterRGB));
          ...
          LBSP::computeRGBDescriptor(oInputImg, anCurrColor, nCurrImgCoord_X,
                                     ...
                                     anRandInputDesc);
          for (size_t c = 0; c < 3; ++c)
            *(m_voBGColorSamples[nSampleModelIdx].data + nPxIterRGB + c) =
                anCurrColor[c];
        }
        if ((rand() % nLearningRate) == 0) {
          int nSampleImgCoord_Y, nSampleImgCoord_X;
          ...
          const size_t nSampleModelIdx = rand() % m_nBGSamples;
          ushort* anRandInputDesc =
              ((ushort*)(m_voBGDescSamples[nSampleModelIdx].data +
                         desc_row_step * nSampleImgCoord_Y +
                         6 * nSampleImgCoord_X));
          const size_t anCurrIntraLBSPThresholds[3] = {
              m_anLBSPThreshold_8bitLUT[anCurrColor[0]],
              m_anLBSPThreshold_8bitLUT[anCurrColor[1]],
          ...
          LBSP::computeRGBDescriptor(oInputImg, anCurrColor, nCurrImgCoord_X,
                                     ...
                                     anRandInputDesc);
          for (size_t c = 0; c < 3; ++c)
            *(m_voBGColorSamples[nSampleModelIdx].data +
              img_row_step * nSampleImgCoord_Y + 3 * nSampleImgCoord_X + c) =
                anCurrColor[c];
        }
      }
    }
...
void BackgroundSubtractorLOBSTER::getBackgroundImage(
...
void BackgroundSubtractorLOBSTER::getBackgroundDescriptorsImage(
...
void BackgroundSubtractorLOBSTER::apply(cv::InputArray image,
