...
SSContext* ss_create(void* img_data, SubtractionMethod method, int width,
                     int height, float lbsp_thresh,
                     size_t desc_dist_thresh_offset,
                     size_t min_color_dist_thresh, size_t num_bg_samples,
  ...
  if (method == SubtractionMethod::SuBSENSE) {
    ctx->subtractor = new BackgroundSubtractorSuBSENSE(
        lbsp_thresh, desc_dist_thresh_offset, min_color_dist_thresh,
        num_bg_samples, num_req_bg_samples, num_samples_for_moving_avg);
  } else if (method == SubtractionMethod::LOBSTER) {
    ctx->subtractor =
        new BackgroundSubtractorLOBSTER(lbsp_thresh, desc_dist_thresh_offset);
  } else {
...
void ss_destroy(SSContext* ctx) { delete ctx; }
