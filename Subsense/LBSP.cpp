...
int LBSP::descriptorSize() const { return DESC_SIZE; }

int LBSP::descriptorType() const { return CV_16U; }

bool LBSP::isUsingRelThreshold() const { return !m_bOnlyUsingAbsThreshold; }

float LBSP::getRelThreshold() const { return m_fRelThreshold; }

size_t LBSP::getAbsThreshold() const { return m_nThreshold; }

static inline void lbsp_computeImpl(
    const cv::Mat& oInputImg, const cv::Mat& oRefImg,
    const std::vector<cv::KeyPoint>& voKeyPoints, cv::Mat& oDesc, size_t _t) {
  CV_DbgAssert(oRefImg.empty() || (oRefImg.size == oInputImg.size &&
                                   oRefImg.type() == oInputImg.type()));
...
static inline void lbsp_computeImpl(
    const cv::Mat& oInputImg, const cv::Mat& oRefImg,
    const std::vector<cv::KeyPoint>& voKeyPoints, cv::Mat& oDesc,
    float fThreshold, size_t nThresholdOffset) {
  CV_DbgAssert(oRefImg.empty() || (oRefImg.size == oInputImg.size &&
                                   oRefImg.type() == oInputImg.type()));
  CV_DbgAssert(oInputImg.type() == CV_8UC1 || oInputImg.type() == CV_8UC3);
  CV_DbgAssert(LBSP::DESC_SIZE ==
...
#include "LBSP_16bits_dbcross_1ch.i"
    ...
    for (size_t k = 0; k < nKeyPoints; ++k) {
      ...
      const uchar* _ref = _refdata + _step_row * (_y) + 3 * (_x);
      ushort* _res = ((ushort*)(oDesc.data + oDesc.step.p[0] * k));
      const size_t _t[3] = {(size_t)(_ref[0] * fThreshold) + nThresholdOffset,
                            (size_t)(_ref[1] * fThreshold) + nThresholdOffset,
                            (size_t)(_ref[2] * fThreshold) + nThresholdOffset};
#include "LBSP_16bits_dbcross_3ch3t.i"
    }
  }
}

static inline void lbsp_computeImpl2(
    const cv::Mat& oInputImg, const cv::Mat& oRefImg,
    const std::vector<cv::KeyPoint>& voKeyPoints, cv::Mat& oDesc, size_t _t) {
  CV_DbgAssert(oRefImg.empty() || (oRefImg.size == oInputImg.size &&
                                   oRefImg.type() == oInputImg.type()));
...
#include "LBSP_16bits_dbcross_1ch.i"
    ...
    for (size_t k = 0; k < nKeyPoints; ++k) {
      ...
      const uchar* _ref = _refdata + _step_row * (_y) + 3 * (_x);
      ushort* _res =
          ((ushort*)(oDesc.data + oDesc.step.p[0] * _y + oDesc.step.p[1] * _x));
#include "LBSP_16bits_dbcross_3ch1t.i"
    }
  }
}
...
void LBSP::compute2(const cv::Mat& oImage,
  ...
  cv::KeyPointsFilter::runByImageBorder(voKeypoints, oImage.size(),
                                        PATCH_SIZE / 2);
  cv::KeyPointsFilter::runByKeypointSize(voKeypoints,
  ...
  if (m_bOnlyUsingAbsThreshold)
    lbsp_computeImpl2(oImage, m_oRefImage, voKeypoints, oDescriptors,
                      m_nThreshold);
  else
    lbsp_computeImpl2(oImage, m_oRefImage, voKeypoints, oDescriptors,
                      m_fRelThreshold, m_nThreshold);
}
...
void LBSP::computeImpl(const cv::Mat& oImage,
                       std::vector<cv::KeyPoint>& voKeypoints,
                       cv::Mat& oDescriptors) const {
  CV_Assert(!oImage.empty());
  cv::KeyPointsFilter::runByImageBorder(voKeypoints, oImage.size(),
  ...
  if (m_bOnlyUsingAbsThreshold)
    lbsp_computeImpl(oImage, m_oRefImage, voKeypoints, oDescriptors,
                     m_nThreshold);
  else
    lbsp_computeImpl(oImage, m_oRefImage, voKeypoints, oDescriptors,
                     m_fRelThreshold, m_nThreshold);
...
void LBSP::reshapeDesc(cv::Size oSize,
  ...
  CV_DbgAssert(oDescriptors.type() == CV_16UC1 ||
               oDescriptors.type() == CV_16UC3);
  ...
  if (nChannels == 1) {
    ...
    for (size_t k = 0; k < nKeyPoints; ++k) {
      ushort* output_ptr =
          (ushort*)(oOutput.data +
                    oOutput.step.p[0] * (int)voKeypoints[k].pt.y);
      const ushort* const desc_ptr =
          (ushort*)(oDescriptors.data + oDescriptors.step.p[0] * k);
      const size_t idx = 3 * (int)voKeypoints[k].pt.x;
      for (size_t n = 0; n < 3; ++n) output_ptr[idx + n] = desc_ptr[n];
    }
  }
}

void LBSP::calcDescImgDiff(const cv::Mat& oDesc1, const cv::Mat& oDesc2,
                           cv::Mat& oOutput, bool bForceMergeChannels) {
  CV_DbgAssert(oDesc1.size() == oDesc2.size() &&
               oDesc1.type() == oDesc2.type());
  CV_DbgAssert(DESC_SIZE == 2);  // @@@ also relies on a constant desc size
  ...
  CV_DbgAssert(oDesc1.step.p[0] == oDesc2.step.p[0] &&
               oDesc1.step.p[1] == oDesc2.step.p[1]);
  ...
  if (nChannels == 1) {
    ...
    for (int i = 0; i < oDesc1.rows; ++i) {
      ...
      for (int j = 0; j < oDesc1.cols; ++j)
        oOutput.at<uchar>(i, j) =
            (uchar)(fScaleFactor * hdist(desc1_ptr[j], desc2_ptr[j]));
    }
  } else {  // nChannels==3
    ...
    for (int i = 0; i < oDesc1.rows; ++i) {
      ...
      for (int j = 0; j < oDesc1.cols; ++j) {
        for (size_t n = 0; n < 3; ++n) {
          ...
          if (bForceMergeChannels)
            output_ptr[j] += (uchar)((fScaleFactor *
                                      hdist(desc1_ptr[idx2], desc2_ptr[idx2])) /
                                     3);
          else
            output_ptr[idx2] =
                (uchar)(fScaleFactor * hdist(desc1_ptr[idx2], desc2_ptr[idx2]));
        }
      }
    }
  }
...
void LBSP::validateKeyPoints(std::vector<cv::KeyPoint>& voKeypoints,
                             cv::Size oImgSize) {
...
void LBSP::validateROI(cv::Mat& oROI) {
  ...
  const cv::Rect nROI_inner(nBorderSize, nBorderSize,
                            oROI.cols - nBorderSize * 2,
                            oROI.rows - nBorderSize * 2);
