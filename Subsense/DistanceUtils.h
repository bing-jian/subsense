...
template <typename T>
static inline typename std::enable_if<std::is_integral<T>::value, size_t>::type
L1dist(T a, T b) {
...
static inline auto L1dist(const T* a, const T* b) -> decltype(L1dist(*a, *b)) {
  decltype(L1dist(*a, *b)) oResult = 0;
  for (size_t c = 0; c < nChannels; ++c) oResult += L1dist(a[c], b[c]);
  return oResult;
}

//! computes the L1 distance between two generic arrays
template <size_t nChannels, typename T>
static inline auto L1dist(const T* a, const T* b, size_t nElements,
                          const uchar* m = NULL)
    -> decltype(L1dist<nChannels>(a, b)) {
  ...
  if (m) {
    for (size_t n = 0, i = 0; n < nTotElements; n += nChannels, ++i)
      if (m[i]) oResult += L1dist<nChannels>(a + n, b + n);
  } else {
...
template <typename T>
static inline auto L1dist(const T* a, const T* b, size_t nElements,
                          size_t nChannels, const uchar* m = NULL)
    -> decltype(L1dist<3>(a, b, nElements, m)) {
...
template <size_t nChannels, typename T>
static inline auto L1dist_(const cv::Vec<T, nChannels>& a,
                           const cv::Vec<T, nChannels>& b)
    -> decltype(L1dist<nChannels, T>((T*)(0), (T*)(0))) {
...
template <size_t nChannels, typename T>
static inline auto L2sqrdist(const T* a, const T* b)
    -> decltype(L2sqrdist(*a, *b)) {
  decltype(L2sqrdist(*a, *b)) oResult = 0;
  for (size_t c = 0; c < nChannels; ++c) oResult += L2sqrdist(a[c], b[c]);
  return oResult;
}

//! computes the squared L2 distance between two generic arrays
template <size_t nChannels, typename T>
static inline auto L2sqrdist(const T* a, const T* b, size_t nElements,
                             const uchar* m = NULL)
    -> decltype(L2sqrdist<nChannels>(a, b)) {
  decltype(L2sqrdist<nChannels>(a, b)) oResult = 0;
  size_t nTotElements = nElements * nChannels;
  if (m) {
    for (size_t n = 0, i = 0; n < nTotElements; n += nChannels, ++i)
      if (m[i]) oResult += L2sqrdist<nChannels>(a + n, b + n);
  } else {
    for (size_t n = 0; n < nTotElements; n += nChannels)
...
static inline auto L2sqrdist(const T* a, const T* b, size_t nElements,
                             size_t nChannels, const uchar* m = NULL)
    -> decltype(L2sqrdist<3>(a, b, nElements, m)) {
  CV_Assert(nChannels > 0 && nChannels <= 4);
  switch (nChannels) {
    case 1:
...
template <size_t nChannels, typename T>
static inline auto L2sqrdist_(const cv::Vec<T, nChannels>& a,
                              const cv::Vec<T, nChannels>& b)
    -> decltype(L2sqrdist<nChannels, T>((T*)(0), (T*)(0))) {
  T a_array[nChannels], b_array[nChannels];
  for (size_t c = 0; c < nChannels; ++c) {
    a_array[c] = a[(int)c];
    b_array[c] = b[(int)c];
...
template <size_t nChannels, typename T>
static inline float L2dist(const T* a, const T* b) {
  decltype(L2sqrdist(*a, *b)) oResult = 0;
  for (size_t c = 0; c < nChannels; ++c) oResult += L2sqrdist(a[c], b[c]);
...
//! computes the L2 distance between two generic arrays
template <size_t nChannels, typename T>
static inline float L2dist(const T* a, const T* b, size_t nElements,
                           const uchar* m = NULL) {
  decltype(L2sqrdist<nChannels>(a, b)) oResult = 0;
  size_t nTotElements = nElements * nChannels;
  if (m) {
...
template <size_t nChannels, typename T>
static inline float L2dist_(const cv::Vec<T, nChannels>& a,
                            const cv::Vec<T, nChannels>& b) {
  T a_array[nChannels], b_array[nChannels];
  for (size_t c = 0; c < nChannels; ++c) {
    a_array[c] = a[(int)c];
...
cdist(const T* curr, const T* bg) {
  ...
  if (bSkip) return (size_t)sqrt((float)curr_sqr);
  size_t bg_sqr = 0;
  size_t mix = 0;
  for (size_t c = 0; c < nChannels; ++c) {
    bg_sqr += bg[c] * bg[c];
...
template <size_t nChannels, typename T>
static inline auto cdist(const T* a, const T* b, size_t nElements,
                         const uchar* m = NULL)
    -> decltype(cdist<nChannels>(a, b)) {
  decltype(cdist<nChannels>(a, b)) oResult = 0;
  size_t nTotElements = nElements * nChannels;
  if (m) {
    for (size_t n = 0, i = 0; n < nTotElements; n += nChannels, ++i)
      if (m[i]) oResult += cdist<nChannels>(a + n, b + n);
  } else {
    for (size_t n = 0; n < nTotElements; n += nChannels)
...
static inline auto cdist(const T* a, const T* b, size_t nElements,
                         size_t nChannels, const uchar* m = NULL)
    -> decltype(cdist<3>(a, b, nElements, m)) {
...
template <size_t nChannels, typename T>
static inline auto cdist_(const cv::Vec<T, nChannels>& a,
                          const cv::Vec<T, nChannels>& b)
    -> decltype(cdist<nChannels, T>((T*)(0), (T*)(0))) {
  T a_array[nChannels], b_array[nChannels];
  for (size_t c = 0; c < nChannels; ++c) {
    a_array[c] = a[(int)c];
    b_array[c] = b[(int)c];
...
template <size_t nChannels, typename T>
static inline typename std::enable_if<std::is_integral<T>::value, size_t>::type
cmixdist(const T* curr, const T* bg) {
...
//! computes the population count of an N-byte vector using an 8-bit popcount
//! LUT
template <typename T>
...
//! computes the hamming distance between two N-byte vectors using an 8-bit
//! popcount LUT
template <typename T>
...
//! computes the gradient magnitude distance between two N-byte vectors using an
//! 8-bit popcount LUT
template <typename T>
...
//! computes the population count of a (nChannels*N)-byte vector using an 8-bit
//! popcount LUT
template <size_t nChannels, typename T>
...
//! computes the hamming distance between two (nChannels*N)-byte vectors using
//! an 8-bit popcount LUT
template <size_t nChannels, typename T>
static inline size_t hdist(const T* a, const T* b) {
  T xor_array[nChannels];
  for (size_t c = 0; c < nChannels; ++c) xor_array[c] = a[c] ^ b[c];
  return popcount<nChannels>(xor_array);
}

//! computes the gradient magnitude distance between two (nChannels*N)-byte
//! vectors using an 8-bit popcount LUT
template <size_t nChannels, typename T>
static inline size_t gdist(const T* a, const T* b) {
